...
public class AdminController {
    ...
    public ResponseEntity<?> salvarItem(@RequestBody ItemCardapio item) {
        try { return ResponseEntity.ok(adminService.salvarItemCardapio(item)); }
        catch (Exception e) { return ResponseEntity.badRequest().body(e.getMessage()); }
    }

    @DeleteMapping("/cardapio/{id}")
    public ResponseEntity<?> deletarItem(@PathVariable Long id) {
        try { adminService.deletarItemCardapio(id); return ResponseEntity.ok().build(); }
        catch (Exception e) { return ResponseEntity.badRequest().body(e.getMessage()); }
    }

    // --- MESAS ---
    @GetMapping("/mesas")
    public ResponseEntity<List<Mesa>> listarMesas() { return ResponseEntity.ok(adminService.listarMesas()); }
    ...
    public ResponseEntity<?> salvarMesa(@RequestBody Mesa mesa) {
        try { return ResponseEntity.ok(adminService.salvarMesa(mesa)); }
        catch (Exception e) { return ResponseEntity.badRequest().body(e.getMessage()); }
    }

    @DeleteMapping("/mesas/{id}")
    public ResponseEntity<?> deletarMesa(@PathVariable Long id) {
        try { adminService.deletarMesa(id); return ResponseEntity.ok().build(); }
        catch (Exception e) { return ResponseEntity.badRequest().body(e.getMessage()); }
    }

    // --- CONFIGURAÇÕES ---
    @GetMapping("/configuracoes")
    public ResponseEntity<Configuracao> getConfiguracoes() { return ResponseEntity.ok(adminService.getConfiguracoes()); }

    @PostMapping("/configuracoes")
    public ResponseEntity<?> salvarConfiguracoes(@RequestBody Configuracao config) {
        try { return ResponseEntity.ok(adminService.salvarConfiguracoes(config)); }
        catch (Exception e) { return ResponseEntity.badRequest().body(e.getMessage()); }
    }

    // --- RELATÓRIOS (CORRIGIDO PARA RECEBER DATAS EM TODOS) ---
    ...
    public ResponseEntity<List<ItemMaisVendidoDTO>> getMaisVendidos(
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime inicio,
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime fim) {
        return ResponseEntity.ok(relatorioService.getItensMaisVendidos(inicio, fim));
    }
    ...
    public ResponseEntity<List<ItemMaiorFaturamentoDTO>> getMaiorFaturamento(
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime inicio,
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime fim) {
        return ResponseEntity.ok(relatorioService.getItensMaiorFaturamento(inicio, fim));
    }
}